...
| **HTML (`auto`)** | Prefer `<picture>`: `<source srcset="…-dark.svg" media="(prefers-color-scheme: dark)">` + `<img src="…svg" alt="…">` so the browser selects one primary resource. Keep `<figure class="mermaid-diagram">` for styling hooks; optional BEM modifiers documented in README. |
